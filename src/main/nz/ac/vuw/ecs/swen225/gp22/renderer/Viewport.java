...
public class Viewport extends JPanel implements ActionListener {
    ...
    public BufferedImage getTileImg(Tile tile) {
        if (tile instanceof Key k) { // check the colours against the four different ones available
            switch (k.getColor()) {
                case Blue:
                    return Img.BlueKey.image;
                case Green:
                    return Img.GreenKey.image;
                case Red:
                    return Img.RedKey.image;
                case Yellow:
                    return Img.YellowKey.image;
                default:
                    throw new IllegalArgumentException("Invalid colour./n");
            }
        }
        if (tile instanceof LockedDoor ld) {
            switch (ld.getColor()) {
                case Blue:
                    return Img.BlueLockedDoor.image;
                case Green:
                    return Img.GreenLockedDoor.image;
                case Red:
                    return Img.RedLockedDoor.image;
                case Yellow:
                    return Img.YellowLockedDoor.image;
                default:
                    throw new IllegalArgumentException("Invalid colour./n");
            }
        }
        if (tile instanceof BouncyPad bp) {
            switch (bp.getDir()) {
                case Up:
                    return Img.BouncyPadUp.image;
                case Down:
                    return Img.BouncyPadDown.image;
                case Left:
                    return Img.BouncyPadLeft.image;
                case Right:
                    return Img.BouncyPadRight.image;
                default:
    ...
    public BufferedImage getEntityImg(Entity.Direction dir) {
        switch (dir) {
            case Up:
                return Img.PlayerUp.image;
            case Down:
                return Img.PlayerDown.image;
            case Left:
                return Img.PlayerLeft.image;
            case Right:
                return Img.PlayerRight.image;
            default:
                throw new IllegalArgumentException("Invalid direction./n");
        }
    }
    ...
    public void checkAction(Entity.Action.Interaction.ActionType a) {
        switch (a) {
            case PickupKey, PickupTreasure -> playSFX("CollectItem", 1);
            case UnlockDoor, UnlockExit -> playSFX("Unlock", 1);
        }
    }
